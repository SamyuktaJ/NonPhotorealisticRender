...
class Dictionary(QtGui.QWidget):
    ...
    def createLayout(self):
        self.bilateral = {
            ...
            'sigmaR': ParameterSlider('Sigma R', 0.1, 10, 0.1, 4.3),
        ...
        self.dog = {
            ...
            'windowSize': ParameterSlider('Window Size', 1, 101, 2, 17),
            'sigmaE': ParameterSlider('Sigma E', 0.1, 10, 0.1, 0.5),
            'tau': ParameterSlider('Tau', 0.8, 1.0, 0.01, 0.98),
    ...
    def getfile(self):
        ...
        self.lea.setPixmap(QtGui.QPixmap())
        
    def quitNPR(self):
        self.moveResult()
        if os.path.isfile('bilateral.yml'):
            os.remove('bilateral.yml')
        if os.path.isfile('conf.csv'):
            os.remove('conf.csv')
        self.close()      
    ...
    def moveResult(self):
        ...
        fname = glob.glob('{}*.png'.format(name))
        ...
        for f in fname:
            shutil.move(f, os.path.join(dirname, os.path.basename(f)))
